...
# ---- geometry / grid ------------------------------------------------------
COLS = int(os.environ.get("WORDMARK_COLS", 96))
...
TEXT = os.environ.get("WORDMARK_TEXT", "AYUSHMAN")

MASK_H = 320           # glyph raster height in mask px (drives voxel density)
TRACKING = 0.18        # extra letter-spacing, in em. counter gaps must survive the
...
DEPTH_FRAC = 0.28      # extrusion depth as a fraction of glyph height
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">Ayushman@github: ~$ ./wordmark.sh --3d</text>')
